...
def search_bar(recommendations, trending_hashtags, user_search_history_data):
    ...
    while True:
        search_query = input(
            "Enter your search query (or type 'exit' to return to the main menu): "
        ).lower()
        if search_query == "exit":
            break

        matching_recommendations = list(
            set(tag for tag in recommendations if search_query in tag)
        )
        matching_trending = list(
            set(tag for tag in trending_hashtags if search_query in tag)
        )
        ...
        # Search for products in product_data
        matching_products = []
        for key, value in product_data.items():
            if search_query in key:
                ...
                for subkey, subvalue in value.items():
                    if search_query in subkey:
                        matching_products.append((key, {subkey: subvalue}))
                    elif any(search_query in item.lower() for item in subvalue):
                        matching_products.append((key, {subkey: subvalue}))
        ...
        if not combined_matches and not matching_products and not history_displayed:
            print(
                "No matching recommendations, trending hashtags, or search history products found."
            )
